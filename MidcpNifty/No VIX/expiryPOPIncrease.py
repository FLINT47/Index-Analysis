...
# Generate Wednesdays as expiry dates from January 24, 2022 to August 17, 2023
wednesdays = pd.date_range(start='2022-01-24', end='2023-08-17', freq='W-WED')

# Generate Mondays as expiry dates from August 21, 2023 onwards
mondays = pd.date_range(start='2023-08-21', end='2024-05-14', freq='W-MON')

# Combine both ranges
expiry_dates = wednesdays.union(mondays)

# Adjust for market holidays: If an expiry date is missing in the data, use the previous business day
adjusted_expiry_dates = [date - BDay(1) if date not in data_from_2022['date'].values else date for date in expiry_dates]
